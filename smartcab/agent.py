...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        self.epsilon = self.epsilon*math.exp(-0.02)
    ...
    def choose_action(self, state):
        ...
        if not self.learning:
            ...
            random_pivot = random.random()
            if random_pivot < self.epsilon:
                action = random.choice(self.valid_actions)
            else:
                # Otherwise, choose an action with the highest Q-value for the current state
                action, maxQ = self.get_maxQ(self.state)
    ...
    def learn(self, state, action, reward):
        ...
        if self.learning:
            ...
            self.Q[state][action] = ((1-alpha) * Q) + (alpha*reward)
...
def run():
    ...
    agent = env.create_agent(
        LearningAgent, learning=True, alpha=0.65, epsilon=1)
